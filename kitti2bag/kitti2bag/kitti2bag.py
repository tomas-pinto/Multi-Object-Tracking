...
def save_tracklet_data(bag, tracklets, velo_frame_id, timestamps, topic):
    ...
    for tid, tracklet in enumerate(tracklets):
        ...
        for translation, rotation, state, occlusion, truncation, amtOcclusion, amtBorders, absoluteFrameNumber in tracklet:
            ...
            marker.action = 0
            if (absoluteFrameNumber - tracklet.firstFrame == tracklet.nFrames-1):
                marker.action = 2
            marker.text = tracklet.objectType
            marker.header.frame_id = velo_frame_id
            ...
            marker.pose.position.z = translation[2] + h/2
            rotq = R.from_rotvec([rotation[0], rotation[1], rotation[2]]).as_quat()
            ...
            marker = Marker()
            marker.id = text_marker_id + tid
            marker.type = Marker.TEXT_VIEW_FACING
            marker.action = 0
            if (absoluteFrameNumber - tracklet.firstFrame == tracklet.nFrames-1):
                marker.action = 2
            marker.text = tracklet.objectType + " " + str(tid)
            marker.header.frame_id = velo_frame_id
            marker.header.stamp = rospy.Time.from_sec(float(timestamp.strftime("%s.%f")))
            marker.pose.position.x = translation[0]
            marker.pose.position.y = translation[1]
            marker.pose.position.z = translation[2] + h/2 + 1
            rotq = R.from_rotvec([rotation[0], rotation[1], rotation[2]]).as_quat()
            marker.pose.orientation.x = rotq[0]
            marker.pose.orientation.y = rotq[1]
            marker.pose.orientation.z = rotq[2]
